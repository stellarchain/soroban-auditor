...
pub struct UnwrapTryFromValIfPattern;

impl UnwrapTryFromValIfPattern {
    pub fn new() -> Self {
        Self
    }
}

impl Pattern for UnwrapTryFromValIfPattern {
    fn name(&self) -> &'static str {
        "unwrap_try_from_val_if"
    }
    ...
    fn apply(&self, block: &FunctionBlock) -> Option<FunctionBlock> {
        ...
        while i < body.len() {
            ...
            if !is_try_from_val_ok_if(header) {
                i += 1;
                continue;
            }
...
fn is_try_from_val_ok_if(line: &str) -> bool {
    line.starts_with("if ")
        && line.ends_with(" {")
        && line.contains("::try_from_val(")
        && line.contains(".is_ok()")
        && !line.contains("!(")
}
...
mod tests {
    ...
    #[test]
    fn unwraps_positive_try_from_val_if() {
        let p = UnwrapTryFromValIfPattern::new();
    ...
    #[test]
    fn keeps_negated_try_from_val_if() {
        let p = UnwrapTryFromValIfPattern::new();
        let block = FunctionBlock {
            header: "pub fn f() {".to_string(),
            body: vec![
